...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../../01_data_preprocessing/01_organise__transition__ews/output_data/02_prepare_roc_data/prepare_roc_data.csv', header=0)
    df_pd = df[df['type'] == 'pd']
    ...
    if ID_merge_tsid == 'all':
        list_tsid_forced = df_pd['tsid'].unique()                           # Select all tsid
    else:
    ...
    for tsid in list_tsid_forced:
        ...
        transition = df_pd[df_pd['tsid'] == tsid].shape[0]
    ...
    if ID_merge_tsid == 'all':
        list_tsid_null = df_pd['tsid'].unique()                             # Select all tsid
    else:
        if isinstance([ID_merge_tsid][0], str):
            # 如果是字符串列表，使用 ast.literal_eval 进行解析
