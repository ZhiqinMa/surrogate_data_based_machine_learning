...
if __name__ == '__main__':
    ...
    filename = '../../01_data_preprocessing/01_organise__transition__ews/output_data/01_ews/df_ews_chick_heart.csv'
    # load the csv file as a data frame
    df_ews = read_csv(filename, header=0)
    ...
    df_ews_column_time = df_ews.columns[0]
    df_ews_column_state = df_ews.columns[1]

    # select columns
    dataframe = df_ews[[df_ews_column_time, df_ews_column_state]][(df_ews['tsid'] == tsid) & (df_ews[df_ews_column_time] <= transition)].reset_index(drop=True)
    # review shape
