...
if __name__ == '__main__':
    ...
    df = pd.read_csv('../../01_data_preprocessing/01_organise__transition__ews/data/04_prepare_roc_data/prepare_roc_data.csv', header=0)
    ...
    eval_pts = np.arange(0.61, 1.01, 0.01)      # percentage of way through pre-transition time series: 20
    # eval_pts = np.arange(0.91, 1.01, 0.01)    # percentage of way through pre-transition time series: 10 for var and corr
    ...
    for tsid in list_tsid_forced:
        df_spec = df_forced[df_forced['tsid'] == tsid].set_index(column_0)
        transition = df_forced[df_forced['tsid'] == tsid].shape[0]
        series = df_spec['state']

        # Compute EWS
        ts = ewstools.TimeSeries(series, transition=transition)
        # ts.detrend(method='Gaussian', span=0.5)
        ts.detrend(method='Gaussian', bandwidth=bandwidth)
        # ts.detrend(method='Gaussian', bandwidth=dict_bandwidth[str(tsid)])

        # ts.compute_var(rolling_window=dict_rw[str(tsid)]/len(df_spec['state']))
        # ts.compute_auto(rolling_window=dict_rw[str(tsid)]/len(df_spec['state']), lag=1)
    ...
    for tsid in list_tsid_null:
        df_spec = df_null[df_null['tsid'] == tsid].set_index(column_0)
