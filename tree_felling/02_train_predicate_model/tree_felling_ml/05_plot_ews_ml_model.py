...
ML_model = str(sys.argv[6])             ## (Set it up to your needs)
...
# load dataset
df_transitions = pd.read_csv('../../01_data_preprocessing/01_organise__transition__ews/data/02_transitions/tree_felling_transitions.csv', header=0)
df_ews = pd.read_csv('../../01_data_preprocessing/01_organise__transition__ews/data/03_ews/tree_felling_df_ews.csv', header=0)
df_SD_probability_mean = pd.read_csv("04_prediction_data/prediction_probability_mean_surrogate_{}_simples_{}_test_{}_{}.csv".format(surr_type, simples, ID_test, ML_model), header=None)
df_SD_probability_error = pd.read_csv("04_prediction_data/prediction_probability_error_surrogate_{}_simples_{}_test_{}_{}.csv".format(surr_type, simples, ID_test, ML_model), header=None)
...
transition_start = df_transitions['Transition_start'][df_transitions['Period'] == ID_test].iloc[0]

# recoed transition start
transition_end = df_transitions['Transition_end'][df_transitions['Period'] == ID_test].iloc[0]
print('start_age, transition_start, transition_end:', start_age, transition_start, transition_end)

# select columns
df_transitions = df_transitions[['Age', 'tree_felling']][(df_transitions['Period'] == ID_test)]
# print(df_transitions)

# record
x_transitions = df_transitions['Age']
y_transitions = df_transitions['tree_felling']
...
for t in range(len(df_SD_probability_mean)):
    value_mean.append(df_transitions.iloc[t + sw_width - 1, 0])
...
for t in range(len(df_SD_probability_error)):
    value_error.append(df_transitions.iloc[t + sw_width - 1, 0])
...
df_SD_probability.to_csv("05_ews_data_and_figures/data/{}/prediction_probability_surrogate_{}_simples_{}_test_{}_{}.csv".format(surr_type, surr_type, simples, ID_test, ML_model), header=True, index=False)
...
ax[0].set_ylabel('Construction activity', fontsize=14)
...
# save figure
plt.savefig('05_ews_data_and_figures/figures/{}/{}_surrogate_{}_train_{}_test_{}_simples_{}_{}.png'.format(surr_type, Variable_label, surr_type, ID_train, ID_test, simples, ML_model), dpi=300)
# Show plot
# plt.show()
