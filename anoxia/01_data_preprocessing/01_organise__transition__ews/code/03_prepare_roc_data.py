...
try:
    os.mkdir('../data/03_prepare_roc_data')
except:
    print('data/03_prepare_roc_data directory already exists!')
print("\n")


# Load in trajectory data
df = pd.read_csv('../data/02_ews/anoxia_df_ews.csv', header=0)
...
for tsid in list_tsid:
    df_before_transition = df[[column_Age, column_state, column_tsid]][(df['tsid'] == tsid) & (df['Variable_label'] == 'Mo')].reset_index(drop=True)
    ...
    df_forced.reset_index(drop=True, inplace=True)

    # Adds the current index as a new column
    df_null['index_column'] = df_null.index
    df_forced['index_column'] = df_forced.index
    # print(df_null)
    # print(df_null.index.name)
...
prepare_roc_data.to_csv('../data/03_prepare_roc_data/prepare_roc_data.csv', index=False)
